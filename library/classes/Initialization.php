...
class Initialization {
	...
	function body_class( $classes ) {
		...
		if ( is_user_logged_in() ) {
			$hide_search            = get_user_meta( get_current_user_id(), 'user_hide_search', true );
			$profile_pic_visibility = get_user_meta( get_current_user_id(), 'profile_picture_visibility', true );
			$search_engine = ! empty( get_user_meta( get_current_user_id(), 'search_engine_selection', true ) ) ? get_user_meta( get_current_user_id(), 'search_engine_selection', true ) : 'google';

			$classes[] = "has-search-engine-{$search_engine}";

			if ( empty( $profile_pic_visibility ) || 'use_empty' === $profile_pic_visibility ) {
				$classes[] = 'use-empty-profile-picture';
			}

			if ( 'use_multiavatar' === $profile_pic_visibility ) {
				$classes[] = 'use-multiavatar-profile-picture';
			}

			if ( '1' === $hide_search ) {
				$classes[] = 'has-search-hidden';
			}
		}
	...
	function wp_title( $title, $sep ) {
		...
		if ( $paged >= 2 || $page >= 2 ) {
			$title .= " $sep " . sprintf( __( 'Page %s', 'oppijaportaali' ), max( $paged, $page ) );
		}
			...
			if ( current_user_can( 'administrator' ) ) {
				add_action( 'admin_notices', create_function( '',
					"echo '<div class=\"error\"><p>" . sprintf( __( 'Please make sure your <a href="%s">.htaccess</a> file is writable ',
						'oppijaportaali' ), admin_url( 'options-permalink.php' ) ) . "</p></div>';" ) );
			}
		}
	}
	*/
